"""End-to-end VPN validation against SRS Phase 1 acceptance criteria (Milestone 11).
...
@pytest.fixture
def vpn_validation(tmp_path: Path):
...
def test_ac01_exactly_one_vpn_experiment_definition_exists() -> None:
    vpn_experiments = list((REPO_ROOT / "experiments").glob("ZC-VPN-EXP-*.json"))
    assert len(vpn_experiments) == 1
    # NOTE: AC-01 requires the experiment be *approved*. ZC-VPN-EXP-001 is still draft
    # pending D-01 (Project Lead sign-off on the anchor CVE pattern) - not yet satisfied.


def test_ac02_experiment_links_to_cve_evidence_and_test_set(vpn_validation) -> None:
    experiment, _repo, _result = vpn_validation
...
def test_ac03_all_valid_cases_accepted_in_both_modes(vpn_validation) -> None:
    _experiment, _repo, result = vpn_validation
...
def test_ac04_mitigation_blocks_malformed_conditions_before_processing(vpn_validation) -> None:
    _experiment, _repo, result = vpn_validation
    dataset = json.loads(DATASET_PATH.read_text(encoding="utf-8"))
    malformed_ids = {
        c["case_id"] for c in dataset["cases"] if c["category"] == TestCaseCategory.MALFORMED.value
    }
    mitigation_malformed = [r for r in result.execution.mitigation.case_results if r.case_id in malformed_ids]
...
def test_ac05_baseline_and_mitigation_use_identical_dataset(vpn_validation) -> None:
    _experiment, _repo, result = vpn_validation
...
def test_ac06_every_mitigation_rejection_is_logged(vpn_validation) -> None:
    _experiment, _repo, result = vpn_validation
...
def test_ac07_comparison_includes_effectiveness_fp_fn_parser_reach_latency(vpn_validation) -> None:
    _experiment, _repo, result = vpn_validation
...
def test_ac08_both_evidence_manifests_pass_integrity_verification(vpn_validation) -> None:
    _experiment, repo, result = vpn_validation
    baseline_manifest = repo.load_manifest("ZC-VPN-EXP-001", result.execution.baseline.run.run_id)
    mitigation_manifest = repo.load_manifest("ZC-VPN-EXP-001", result.execution.mitigation.run.run_id)
    assert verify_manifest_integrity(baseline_manifest) is True
    assert verify_manifest_integrity(mitigation_manifest) is True


def test_ac09_no_unsafe_execution_configuration(vpn_validation) -> None:
    experiment, _repo, _result = vpn_validation
...
def test_qg3_baseline_demonstrates_expected_weakness_safely(vpn_validation) -> None:
    _experiment, _repo, result = vpn_validation
...
def test_qg4_mitigation_passes_with_valid_traffic_preserved(vpn_validation) -> None:
    _experiment, _repo, result = vpn_validation
...
def test_qg5_manifest_raw_results_and_comparison_all_verified(vpn_validation) -> None:
    _experiment, repo, result = vpn_validation
